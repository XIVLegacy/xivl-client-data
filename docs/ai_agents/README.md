...
change. The contributor owns the result, can explain the claim or edit, and
can name the checks that support it.

## Contribution policy

- Keep `csv/` as the as-imported extraction. Never hand-edit, re-emit, or
  normalize its rows.
...
- Preserve extraction versions, source paths, evidence classes, dates, and
  stable citations verbatim when they identify an input or claim.
...
- Do not add client binaries, capture archives, credentials, database dumps,
  or generated build output to the tracked surface.

Evidence and claim rules are in [Evidence and claims](evidence-and-claims.md).
The repository gate and its limits are in [Verification](verification.md).
